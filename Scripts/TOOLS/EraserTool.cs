...
public class EraserTool : Tool {
	...
	void Update () {
		...
		if(Input.GetMouseButtonUp(1)) { OnDeselect();} //Deselect eraser tool.

		if(Input.GetMouseButtonDown(0)) { IsBusy = true;} // Start an erase round

		// Erasing
		if(IsBusy) { Erase ();}

		if(Input.GetMouseButtonUp(0)) { IsBusy = false; } //Finish an erase round. The tool is free and can be erased again
	}
